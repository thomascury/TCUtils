...
def create_socket(source_ip, dest_ip):
    ...
    try:
        ...
        print('Socket could not be created.  Error: %s Message: %s' % (str(msg.args[0]), msg.args[1]))
        sys.exit()
...
for i in range(len(open_port_list)):
    print('Process #: %s Open ports: %s' % (i, open_port_list[i]))
...
sudo ./pyscan.py
Process #:  0  Open ports:  []
Process #:  1  Open ports:  [135, 139]
Process #:  2  Open ports:  []
Process #:  3  Open ports:  []
Process #:  4  Open ports:  []
Process #:  5  Open ports:  []
Process #:  6  Open ports:  []
Process #:  7  Open ports:  []
Process #:  8  Open ports:  []
Process #:  9  Open ports:  [445]
A list of all open ports found:
135 ,
139 ,
445 ,

Now all I need to do is make the calls threaded with a neat final report at the end.
'''